...
function limparHtml(value = '') {
  return value.replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ').replace(/<[^>]+>/g, ' ').replace(/&nbsp;/gi, ' ').replace(/&amp;/gi, '&').replace(/&quot;/gi, '"').replace(/&#(?:x([0-9a-f]+)|(\d+));/gi, (_, hex, decimal) => String.fromCharCode(parseInt(hex || decimal, hex ? 16 : 10))).replace(/\s+/g, ' ').trim();
}
function urlAbsoluta(value) { try { return new URL(String(value).replace(/&amp;/gi, '&'), BASE_URL).toString(); } catch { return null; } }
async function buscar(url, binary = false) {
  const response = await fetch(url, { headers: { 'User-Agent': 'AutoVagas/1.0 (consulta pública de editais)', Accept: binary ? 'application/pdf,*/*' : 'text/html,application/xhtml+xml' }, signal: AbortSignal.timeout(20000) });
...
function extrairLinksDeEditais(html) {
  const aberto = html.match(/Em\s+andamento([\s\S]*?)(?:Encerrados|<h[1-6][^>]*>\s*Encerrados)/i)?.[1] || html;
  const links = []; const regex = /<a\b[^>]*href=["']([^"']*\/visualizar\/\d+[^"']*)["'][^>]*>([\s\S]*?)<\/a>/gi; let match;
  while ((match = regex.exec(aberto))) {
    const url = urlAbsoluta(match[1]); const texto = limparHtml(match[2]); const id = url?.match(/\/visualizar\/(\d+)/)?.[1];
    if (!url || !id || !texto || links.some((item) => item.id === id)) continue;
    links.push({ id, url, titulo: texto.split(/Inscri[cç][õo]es\s+at[eé]/i)[0].trim() || texto, resumo: texto, prazo: texto.match(/Inscri[cç][õo]es\s+at[eé]\s+([0-3]?\d\/\d{2}\/\d{4})/i)?.[1] || null });
  }
  return links;
}
...
function extrairLinksAnexos(html) {
  const links = []; const regex = /<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi; let match;
  while ((match = regex.exec(html))) {
    const url = urlAbsoluta(match[1]); const nome = limparHtml(match[2]);
    if (!url || !/(download|arquivo|anexo|edital|\.pdf(?:$|\?))/i.test(`${url} ${nome}`) || links.some((item) => item.url === url)) continue;
    links.push({ nome: nome || 'Anexo do edital', url, tipo: tipoAnexo(nome || url) });
  }
  return links.slice(0, 8);
}
function trecho(texto, expressao, tamanho = 850) { const found = expressao.exec(texto); return found ? texto.slice(found.index, found.index + tamanho).replace(/\s+/g, ' ').trim() : null; }
...
function datasNoTexto(texto) {
  const datas = [];
  const regex = /\b([0-3]?\d\/\d{2}\/\d{4}|([0-3]?\d)\s+de\s+(janeiro|fevereiro|mar[cç]o|abril|maio|junho|julho|agosto|setembro|outubro|novembro|dezembro)\s+de\s+(\d{4}))\b/gi;
  let match;
...
function extrairDataFinalInscricao(texto) {
  const contextos = texto.match(/(?:inscri[cç](?:[aã]o|[õo]es?)|prazo\s+(?:para\s+)?inscri[cç](?:[aã]o|[õo]es?))[\s\S]{0,420}/gi) || [];
...
function extrairPeriodoVaga(texto) {
  const match = texto.match(/(?:per[ií]odo\s+de\s+vig[eê]ncia|vig[eê]ncia\s+(?:da\s+)?bolsa|dura[cç][aã]o\s+(?:da\s+)?bolsa|per[ií]odo\s+da\s+bolsa)\s*[:\-]?\s*(.{0,110}?\b(?:dias?|meses?|anos?)\b)/i);
  return match ? match[0].replace(/\s+/g, ' ').trim() : null;
}
function extrairProcesso(texto, fallback) { return texto.match(/(?:processo\s+seletivo(?:\s+simplificado)?|edital\s+(?:de\s+)?sele[cç][aã]o)[^.!?]{0,180}/i)?.[0]?.trim() || fallback; }
...
function possuiListaDeCandidatos(texto, indiceResultado) {
  ...
  const temCabecalho = /(?:nome\s+(?:do|d[ao]s?)?\s*candidat|lista\s+(?:de\s+)?(?:candidat|inscrit|aprovad|classificad|homologad)|candidat(?:os|as)?\s+(?:aprovad|classificad|homologad)|classifica[cç][aã]o\s+(?:final|preliminar))/i.test(amostra);
  if (!temCabecalho) return false;
  const nomes = amostra.match(/\b[A-ZÀ-Ý]{2,}(?:\s+(?:DE|DA|DO|DAS|DOS|E))?(?:\s+[A-ZÀ-Ý]{2,}){1,4}\b/g) || [];
  const ignorar = /MINISTÉRIO|EDUCAÇÃO|UNIVERSIDADE|FEDERAL|RIO GRANDE|INSTITUTO|METRÓPOLE|DIGITAL|PROCESSO|SELETIVO|RESULTADO|EDITAL|ANEXO|CANDIDATO|INSCRIÇÃO|COMISSÃO|CLASSIFICAÇÃO|HOMOLOGAÇÃO|CRONOGRAMA|COORDENAÇÃO|PROJETO/;
...
function identificarResultadoComLista(anexos) {
  for (const anexo of anexos) {
    ...
    const match = /(?:resultado\s+(?:preliminar|parcial|final|definitivo)|homologa[cç][aã]o\s+(?:das\s+)?inscri[cç][õo]es|lista\s+(?:de\s+)?(?:candidat|aprovad|classificad|homologad)|classifica[cç][aã]o\s+(?:final|preliminar))/i.exec(cabecalho);
    if (!match || !possuiListaDeCandidatos(texto, match.index)) continue;
    return { url: anexo.url, situacao: /resultado\s+(?:final|definitivo)|classifica[cç][aã]o\s+final/i.test(match[0]) ? 'resultado_final_publicado' : 'resultados_parciais_publicados' };
  }
  return null;
}
...
async function detalhar(item) {
  ...
  const anexosComTexto = await Promise.all(extrairLinksAnexos(html).map(async (anexo) => ({ ...anexo, texto: await textoDoPdf(anexo) })));
  ...
  const situacao = dataFinalInscricao
    ? (prazoEncerrado ? (resultadoAnexo ? resultadoAnexo.situacao : 'aguardando_resultados_parciais') : 'inscricoes_abertas')
...
async function listarEditais({ force = false } = {}) {
  ...
  const results = await Promise.allSettled(cards.map(detalhar)); const data = results.filter((result) => result.status === 'fulfilled').map((result) => result.value);
  if (!data.length && cards.length) throw new Error('Não foi possível analisar os editais do IMD neste momento.');
