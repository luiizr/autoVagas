...
function assinaturaDasVagas(vagas) {
  return JSON.stringify(
    vagas.map((vaga) => ({
      uid: vaga.uid,
      prazo: vaga.dataFinalInscricao,
      situacao: vaga.situacao,
      resultado: vaga.resultadoUrl,
      anexos: vaga.anexos?.map((anexo) => anexo.url),
    })),
  );
}
...
async function notificarAssinatura(subscription, vagas) {
  ...
  const novos = compativeis.filter(
    (vaga) => !subscription.editaisEnviados.includes(vaga.notificacaoId || vaga.uid),
  );
  ...
  if (result.sent)
    await marcarEnviados(
      subscription.id,
      novos.map((vaga) => vaga.notificacaoId || vaga.uid),
    );
...
async function atualizarNotificacoes() {
  ...
  for (const subscription of subscriptions.filter((item) => item.ativo && item.consentimento)) {
    try {
      results.push({ id: subscription.id, ...(await notificarAssinatura(subscription, vagas)) });
    } catch (error) {
      results.push({ id: subscription.id, sent: false, error: error.message });
    }
  }
  return results;
}
...
router.get('/atualizacoes', (req, res) => {
  ...
  req.on('close', () => {
    ...
    atualizacoes.off('vagas-atualizadas', onUpdate);
  });
});
...
router.get('/vagas', async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      vagas: onlyGraduation ? vagas.filter((item) => item.ehGraduacao) : vagas,
    });
  } catch (error) {
    next(error);
  }
...
router.get('/editais', async (req, res, next) => {
  try {
    ...
    res.json({
      fonte: 'imd',
      atualizadoEm: new Date().toISOString(),
      editais: req.query.graduacao === 'true' ? vagas.filter((item) => item.ehGraduacao) : vagas,
    });
  } catch (error) {
    next(error);
  }
  ...
  try {
    ...
    res.status(201).json({
      subscription: {
        ...
        fontes: subscription.fontes,
        somenteGraduacao: subscription.somenteGraduacao,
      },
      editais: filtrarVagas(vagas, subscription),
      notification,
    });
  } catch (error) {
    next(error);
...
router.post('/admin/atualizar-notificacoes', async (req, res, next) => {
  try {
    if (!process.env.API_ADMIN_KEY || req.get('x-api-key') !== process.env.API_ADMIN_KEY)
