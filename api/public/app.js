...
const filterInputs = { open: document.querySelector('#filter-open'), result: document.querySelector('#filter-result'), waiting: document.querySelector('#filter-waiting'), deadline: document.querySelector('#filter-deadline'), attachmentData: document.querySelector('#filter-attachment-data'), value: document.querySelector('#filter-value') };
...
function temDadosAproveitaveisDoAnexo(vaga) {
  ...
  return Boolean(vaga.valorVaga || vaga.linkInscricao || vaga.emailInscricao || vaga.numeroVagas || (Array.isArray(vaga.perfisVaga) && vaga.perfisVaga.length));
}
function filtrosAtivos() { return { open: filterInputs.open.checked, result: filterInputs.result.checked, waiting: filterInputs.waiting.checked, deadline: filterInputs.deadline.checked, attachmentData: filterInputs.attachmentData.checked, institution: document.querySelector('input[name="institution"]:checked').value, value: Number(filterInputs.value.value) || 0 }; }
...
function card(vaga) {
  const periodoInscricao = [vaga.dataInicioInscricao ? `De ${vaga.dataInicioInscricao}` : '', vaga.dataFinalInscricao || vaga.prazo ? `até ${vaga.dataFinalInscricao || vaga.prazo}` : ''].filter(Boolean).join(' ') || 'Consulte o anexo';
  const vagas = vaga.numeroVagas ? `${vaga.numeroVagas} vaga${vaga.numeroVagas === '01' || vaga.numeroVagas === '1' ? '' : 's'}${vaga.cadastroReserva ? ` · cadastro reserva: ${vaga.cadastroReserva}` : ''}` : 'Não informado';
  const detalhes = [['Inscrições', periodoInscricao], ['Valor da vaga', vaga.valorVaga || 'Não informado'], ['Vagas', vagas], ['Inscrição', vaga.formaInscricao || 'Consulte o anexo']];
  ...
  const informacoes = [
    ...
    ['Bolsa / jornada', [vaga.tipoBolsa, vaga.cargaHoraria && `${vaga.cargaHoraria} semanais`, vaga.turno, vaga.modalidadeTrabalho].filter(Boolean).join(' · ')],
  ...
  const detalhesPdf = informacoes.length ? `<details class="pdf-details"><summary>Detalhes extraídos do anexo</summary>${informacoes.map(([rotulo, valor]) => `<p><strong>${escapeHtml(rotulo)}:</strong> ${escapeHtml(valor)}</p>`).join('')}</details>` : '';
  const perfis = Array.isArray(vaga.perfisVaga) && vaga.perfisVaga.length ? vaga.perfisVaga : (vaga.atividadesVaga || vaga.conhecimentosNecessarios ? [{ tituloPerfil: 'Perfil da vaga', atividadesVaga: vaga.atividadesVaga, conhecimentosNecessarios: vaga.conhecimentosNecessarios }] : []);
  const experiencia = perfis.length ? `<details class="profile-details"><summary>O que você fará e conhecimentos exigidos (${perfis.length} perfil${perfis.length === 1 ? '' : 'is'})</summary>${perfis.map((perfil) => `<section class="job-profile"><h4>${escapeHtml(perfil.tituloPerfil)}</h4>${perfil.atividadesVaga ? `<p><strong>Atuação:</strong> ${escapeHtml(perfil.atividadesVaga)}</p>` : ''}${perfil.conhecimentosNecessarios ? `<p><strong>Stacks e conhecimentos:</strong> ${escapeHtml(perfil.conhecimentosNecessarios)}</p>` : ''}</section>`).join('')}</details>` : '';
